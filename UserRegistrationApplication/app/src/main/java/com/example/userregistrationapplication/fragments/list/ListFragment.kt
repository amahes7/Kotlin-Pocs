...
// ALl Users using single API call by Flow
        userViewModel.getAllUsersByFlow()
        val resp = userViewModel.result
        if (resp.status.equals(Constants.SUCCESS_TEXT)) {
            resp.body.observe(viewLifecycleOwner, Observer { user ->
